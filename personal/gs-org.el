...
(setq org-todo-keywords
      '((sequence "TODO(t)" "INPROGRESS(n)" "|" "DONE(d)")
...
(setq org-todo-state-tags-triggers
  ...
  ("INPROGRESS" ("WAITING") ("CANCELLED") ("INACTIVE"))
  ("DONE" ("WAITING") ("CANCELLED") ("INACTIVE"))))

(defun gs/mark-next-done-parent-tasks-todo ()
  "Visit each parent task and change INPROGRESS (or DONE) states to TODO."
  ;; Don't change the value if new state is "DONE"
  (let ((mystate (or (and (fboundp 'org-state)
                          (member state
          (list "INPROGRESS" "TODO")))
                     (member (nth 2 (org-heading-components))
           (list "INPROGRESS" "TODO")))))
    (when mystate
      (save-excursion
        (while (org-up-heading-safe)
          (when (member (nth 2 (org-heading-components)) (list "INPROGRESS" "DONE"))
            (org-todo "TODO")))))))
...
;; refile to top level (https://blog.aaronbieber.com/2017/03/19/organizing-notes-with-refile.html)
...
;; If not a project, clocking-in changes TODO to INPROGRESS
...
;; Also ensure that INPROGRESS projects are switched to TODO when clocking in
